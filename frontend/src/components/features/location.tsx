...
export function Location() {
    ...
    const openDirections = (mode?: string) => {
        if (!current) return
        const url = mode
            ? `${current.directions_base}&travelmode=${mode}`
            : current.directions_base
                ...
                <div className={styles.locationButtons}>
                    {locations.map((loc) => (
                        <button
                            key={loc.id}
                            type="button"
                            className={`${styles.locationButton} ${current.id === loc.id ? styles.locationButtonActive : ''}`}
                            onClick={() => setCurrent(loc)}
                        >
                            {loc.location_name} {/* DB에서 가져온 location 이름 */}
                        </button>
                    ))}
                </div>
                ...
                <div className={styles.grid}>
                    ...
                    <div className={styles.main}>
                        <div className={styles.card}>
                            <h2 className={styles.cardTitle}>{current.location_name} Campus</h2>
                            ...
                            <div className={styles.mapContainer}>
                                <MapComponent link={current.map_embed_link} />
                            </div>
