export const DUMMY_POPUP: Record<string, any[]> = {
  'en': [
    {
      id: 'study-habits-1',
      title: 'Elementary Study Habits',
      bodies: [
        "Building strong foundations for future success.",
        "Daily routines that create lifelong learners."
      ]
    },
    {
      id: 'admission-2024',
      title: '2024 All Accepted',
      bodies: [
        "Celebrating 100% acceptance rate for our graduates!",
        "<strong>Your child could be next.</strong>",
      ]
    },
    {
      id: 'study-habits-2',
      title: 'Elementary Study Habits',
      bodies: [
        "Focused attention on individual growth.",
        "Consistent feedback and guidance."
      ]
    }
  ],
  'ko': [
    {
      id: 'study-habits-1-ko',
      title: '저학년 공부습관',
      bodies: [
        "미래의 성공을 위한 탄탄한 기초를 다집니다.",
        "평생 가는 공부 습관을 길러줍니다."
      ]
    },
    {
      id: 'admission-2024-ko',
      title: '2024년 전원 합격',
      bodies: [
        "<strong>졸업생 전원 명문대 합격의 쾌거!</strong>",
        "The Doum Academy와 함께라면 가능합니다."
      ]
    },
    {
      id: 'study-habits-2-ko',
      title: '저학년 공부습관',
      bodies: [
        "개별 성장에 맞춘 집중 케어.",
        "지속적인 피드백과 밀착 지도."
      ]
    }
  ]
};

export const reviews = [
  {
    name: 'Sarah J.',
    grade: 'Graduate',
    university: 'UBC - Science',
    text: 'Devin-seams math classes were a game changer for my AP Calculus exam. The way he explains complex concepts is so clear!',
    rating: 5
  },
  {
    name: 'Kevin L.',
    grade: 'Grade 12',
    university: 'Target: UofT Engineering',
    text: 'The 1:1 roadmap provided by The Doum Academy helped me manage my GPA while preparing for university applications. Highly recommend Kate-seams mentoring!',
    rating: 5
  },
  {
    name: 'Ji-won M.',
    grade: 'Graduate',
    university: 'SFU - Computing Science',
    text: 'Thanks to the coding and STEM classes at Doum, I felt completely prepared for my first year at SFU. The instructors are top-notch.',
    rating: 5
  },
  {
    name: 'David H.',
    grade: 'Parent',
    university: 'Parent of Grade 11 Student',
    text: 'My son has shown incredible progress in his English and Social Studies grades. The customized approach here really works.',
    rating: 5
  },
  {
    name: 'Emily W.',
    grade: 'Graduate',
    university: 'Waterloo - Math',
    text: 'The specific university entrance strategies based on 20 years of data were what made the difference for my application. Thank you Doum!',
    rating: 5
  }
];

export const heroContent = {
  en: {
    title: "Standard of Canadian Admissions,\nThe Doum Academy",
    subtitle: "Beyond simple grades, guiding the way to your child's successful future.",
    cta: "Book an Appointment"
  },
  ko: {
    title: "캐나다 입시의 기준,\nThe Doum Academy",
    subtitle: "단순한 성적을 넘어, 아이의 성공적인 미래로 가는 길을 안내합니다.",
    cta: "상담 예약하기"
  }
};

export const featureBanners = {
  en: [
    {
      title: "Seamless Roadmap",
      description: "Continuous academic guidance from K-12 to University."
    },
    {
      title: "Master Plan",
      description: "Personalized university entrance strategies backed by data."
    },
    {
      title: "Junior Study Habits",
      description: "Building strong foundations and daily routines for lifelong success."
    },
    {
      title: "2024 All Accepted",
      description: "Celebrating 100% acceptance rate for our graduates to top universities."
    }
  ],
  ko: [
    {
      title: "끊김없는 로드맵",
      description: "초등부터 대학까지 이어지는 체계적인 학업 가이드."
    },
    {
      title: "맞춤형 마스터 플랜",
      description: "20년 성공 데이터를 기반으로 한 개인별 입시 전략."
    },
    {
      title: "저학년 공부습관",
      description: "평생 성공을 위한 탄탄한 기초와 올바른 공부 습관 형성."
    },
    {
      title: "2024년 전원 합격",
      description: "졸업생 전원 명문대 합격의 쾌거! The Doum Academy의 자랑입니다."
    }
  ]
};

export const universityLogos = [
  "UBC", "University of Toronto", "Waterloo", "McGill", "SFU", "McMaster", "Western", "Queens", "Alberta", "Calgary"
];